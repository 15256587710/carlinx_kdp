...
import com.ecarxclub.app.utils.banner.adapter.CBPageAdapter;
...
public class CBLoopViewPager extends ViewPager {
    ...
    private OnPageChangeListener onPageChangeListener = new OnPageChangeListener() {
        ...
        @Override
        public void onPageSelected(int position) {//此方法是页面跳转完后得到调用，arg0是你当前选中的页面的Position（位置编号）
            int realPosition = mAdapter.toRealPosition(position);
            if (mPreviousPosition != realPosition) {
                mPreviousPosition = realPosition;
                if (mOuterPageChangeListener != null) {
                    mOuterPageChangeListener.onPageSelected(realPosition);
                    onItemClickListener.onSelectChoose((getRealItem()));
                }
            }
        }
        ...
        @Override
        public void onPageScrollStateChanged(int state) {//1的时辰默示正在滑动，arg0==2的时辰默示滑动完毕了，arg0==0的时辰默示什么都没做
            if (mOuterPageChangeListener != null) {
