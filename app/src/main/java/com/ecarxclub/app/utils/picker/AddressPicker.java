...
import com.ecarxclub.app.utils.picker.listener.OnLinkageListener;
import com.ecarxclub.app.utils.picker.listener.OnMoreWheelListener;
...
public class AddressPicker extends LinkagePicker {
    ...
    public Province.City getSelectedCity() {
        return getSelectedProvince().getChild_list().get(selectedSecondIndex);
    }

    public Province.City.County getSelectedCounty() {
        if (getSelectedCity() != null && getSelectedCity().getChild_list() != null && getSelectedCity().getChild_list().size() != 0) {
            return getSelectedCity().getChild_list().get(selectedThirdIndex);
        } else {
            return null;
        }
        ...
        if (wheelModeEnable) {
            ...
            provinceView.setOnItemPickListener(new OnItemPickListener<String>() {
                ...
                public void onItemPicked(int index, String item) {
                    ...
                    LogUtils.verbose("change cities after province wheeled");
            ...
            cityView.setOnItemPickListener(new OnItemPickListener<String>() {
                ...
                public void onItemPicked(int index, String item) {
                    ...
                    LogUtils.verbose("change counties after city wheeled");
                    selectedThirdIndex = 0;//重置县级索引
                    //根据地市获取区县
                    List<String> counties = provider.provideThirdData(selectedFirstIndex, selectedSecondIndex);
                    if (counties.size() > 0) {
    ...
    public void onSubmit() {
        if (onLinkageListener != null) {
            ...
            Province.City city = getSelectedCity();
    ...
    public static class AddressProvider implements DataProvider {
        ...
        private void parseData(List<Province> data) {
            ...
            for (int x = 0; x < provinceSize; x++) {
                ...
                firstList.add(pro.getRegion_name());
                List<Province.City> cities = pro.getChild_list();
                List<String> xCities = new ArrayList<>();
                List<List<String>> xCounties = new ArrayList<>();
                int citySize = cities.size();
                //添加地市
                for (int y = 0; y < citySize; y++) {
                    Province.City cit = cities.get(y);
//                    cit.setProvinceId(pro.getAreaId());
                    xCities.add(cit.getRegion_name());
                    List<Province.City.County> counties = cit.getChild_list();
                    ArrayList<String> yCounties = new ArrayList<>();
                    int countySize = counties.size();
                    //添加区县
                    if (countySize == 0) {
                        yCounties.add(cit.getRegion_name());
                    } else {
                        for (int z = 0; z < countySize; z++) {
                            Province.City.County cou = counties.get(z);
                            yCounties.add(cou.getRegion_name());
                        }
                    }
