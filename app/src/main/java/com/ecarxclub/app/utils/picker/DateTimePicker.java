package com.ecarxclub.app.utils.picker;
...
import com.ecarxclub.app.utils.LogUtils;
...
import com.ecarxclub.app.utils.picker.utils.TimeUtils;
import com.ecarxclub.app.utils.picker.widget.WheelListView;
...
public class DateTimePicker extends WheelPicker {
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        LogUtils.verbose("change months and days while set selected");
        ...
        if (timeMode != NONE) {
            selectedHour = TimeUtils.fillZero(hour);
            selectedMinute = TimeUtils.fillZero(minute);
        }
    }
    ...
    public void setSelectedItem(int yearOrMonth, int monthOrDay, int hour, int minute) {
        ...
        if (dateMode == MONTH_DAY) {
            ...
            LogUtils.verbose("change months while set selected");
    ...
    protected View makeCenterView() {
        ...
        if ((dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) && years.size() == 0) {
            LogUtils.verbose("init years before make view");
            initYearData();
        }
        if (dateMode != NONE && months.size() == 0) {
            LogUtils.verbose("init months before make view");
            int selectedYear = TimeUtils.trimZero(getSelectedYear());
            changeMonthData(selectedYear);
        }
        if ((dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) && days.size() == 0) {
            LogUtils.verbose("init days before make view");
            int selectedYear;
            if (dateMode == YEAR_MONTH_DAY) {
                selectedYear = TimeUtils.trimZero(getSelectedYear());
            } else {
                selectedYear = Calendar.getInstance(Locale.CHINA).get(Calendar.YEAR);
            }
            int selectedMonth = TimeUtils.trimZero(getSelectedMonth());
            changeDayData(selectedYear, selectedMonth);
        }
        if (timeMode != NONE && hours.size() == 0) {
            LogUtils.verbose("init hours before make view");
            initHourData();
        }
        if (timeMode != NONE && minutes.size() == 0) {
            LogUtils.verbose("init minutes before make view");
            changeMinuteData(TimeUtils.trimZero(selectedHour));
        }
...
//                            return;
//                        }
                        LogUtils.verbose("change months after year wheeled");
                        selectedMonthIndex = 0;//重置月份索引
                        selectedDayIndex = 0;//重置日子索引
                        //需要根据年份及月份动态计算天数
                        int selectedYear = TimeUtils.trimZero(item);
                        changeMonthData(selectedYear);
                        monthView.setAdapter(new ArrayWheelAdapter<>(months));
                        monthView.setCurrentItem(selectedMonthIndex);
                        changeDayData(selectedYear, TimeUtils.trimZero(months.get(selectedMonthIndex)));
...
//                        }
                        if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                            LogUtils.verbose("change days after month wheeled");
                            selectedDayIndex = 0;//重置日子索引
                            int selectedYear;
                            if (dateMode == YEAR_MONTH_DAY) {
                                selectedYear = TimeUtils.trimZero(getSelectedYear());
                            ...
                            changeDayData(selectedYear, TimeUtils.trimZero(item));
                            dayView.setAdapter(new ArrayWheelAdapter<>(days));
                            dayView.setCurrentItem(selectedDayIndex);
                        }
                    }
...
//                layout.setWeightSum(5);
                ...
                hourView.setOnItemPickListener(new OnItemPickListener<String>() {
                    ...
                    public void onItemPicked(int index, String item) {
                        ...
                        changeMinuteData(TimeUtils.trimZero(item));
            ...
            if (timeMode != NONE) {
                ...
                hourView.setOnWheelChangeListener(new WheelListView.OnWheelChangeListener() {
                    ...
                    public void onItemSelected(boolean isUserScroll, int index, String item) {
                        ...
                        LogUtils.verbose("change minutes after hour wheeled");
                        changeMinuteData(TimeUtils.trimZero(item));
        ...
        if (startYear == endYear) {
            if (startMonth > endMonth) {
                for (int i = endMonth; i >= startMonth; i--) {
                    months.add(TimeUtils.fillZero(i));
                }
            } else {
    ...
    private void changeDayData(int selectedYear, int selectedMonth) {
        int maxDays = TimeUtils.calculateDaysInMonth(selectedYear, selectedMonth);
        days.clear();
        if (selectedYear == startYear && selectedMonth == startMonth
                && selectedYear == endYear && selectedMonth == endMonth) {
            //开始年月及结束年月相同情况
            for (int i = startDay; i <= endDay; i++) {
                days.add(TimeUtils.fillZero(i));
            }
    ...
    private void initHourData() {
        for (int i = startHour; i <= endHour; i++) {
            String hour = TimeUtils.fillZero(i);
            hours.add(hour);
        }
    ...
    private void changeMinuteData(int selectedHour) {
        if (startHour == endHour) {
            ...
            for (int i = startMinute; i <= endMinute; i++) {
                minutes.add(TimeUtils.fillZero(i));
            }
