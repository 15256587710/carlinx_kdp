...
public class TabFragmentHost extends TabHost implements
    ...
    public void setup(Context context, FragmentManager manager, int containerId) {
        ...
        // 我们必须有一个能够代表ID保存/恢复我们的状态。如果有人还没见过一这一点上，我们将看到它ourself。
        if (getId() == View.NO_ID) {
    ...
    public void addTab(TabSpec tabSpec, Class<?> clss, Bundle args) {
        ...
        if (mAttached) {
            ...
            // normally happen.
            //如果我们已经连接到窗口，则检查以确保此选项卡的片段在存在时不活动。这通常不会发生。
    ...
    protected void onAttachedToWindow() {
        ...
        // correct tab. 我们现在准备出发了。确保我们切换到   正确的选项卡。
        mAttached = true;
...
//        Log.i("cwp","onTabChanged11 "+tabId+" --  "+mAttached);
        if (mAttached) {
            ...
            if("tab1".equals(tabId)){
                EventBus.getDefault().post(new EventMessage(Constant.MAIN_TAB_CHANGE,Constant.MAIN_TAB_CHANGE_MAME));
            }else if ("tab2".equals(tabId)){
                EventBus.getDefault().post(new EventMessage(Constant.MAIN_TAB_CHANGE,Constant.MAIN_TAB_CHANGE_MAME2));
            }else if ("tab3".equals(tabId)){
                EventBus.getDefault().post(new EventMessage(Constant.MAIN_TAB_CHANGE,Constant.MAIN_TAB_CHANGE_MAME3));
            }else if ("tab4".equals(tabId)){
                EventBus.getDefault().post(new EventMessage(Constant.MAIN_TAB_CHANGE,Constant.MAIN_TAB_CHANGE_MAME4));
            }
        }
