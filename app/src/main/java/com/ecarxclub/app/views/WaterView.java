...
//            0.01f, 0.05f, 0.1f, 0.6f, 0.11f, 0.16f, 0.21f, 0.26f, 0.31f, 0.38f, 0.45f, 0.56f, 0.65f, 0.77f
            0.01f, 0.09f,  0.17f, 0.24f,0.32f, 0.41f, 0.49f, 0.57f,  0.66f, 0.74f
    );
    /**
     * y最多可选取的随机数值
     */
...
//            0.01f, 0.06f, 0.11f, 0.17f, 0.23f, 0.29f, 0.35f, 0.41f, 0.47f, 0.53f, 0.59f, 0.65f, 0.71f, 0.77f, 0.83f
            0.01f, 0.06f, 0.10f, 0.14f, 0.18f, 0.22f, 0.25f, 0.29f
    );
    /**
     * x坐标当前可选的随机数组
     */
     ...
     * @param waters
     */
    public void setWaters(final List<WaterRes.DataBean.BubbleBean> waters) {
     ...
     * @param waters
     */
    private void setDatas(List<WaterRes.DataBean.BubbleBean> waters) {
        reset();
     ...
     * 添加水滴view
     */
    private void addWaterView(List<WaterRes.DataBean.BubbleBean> waters) {
        for (int i = 0; i < waters.size(); i++) {
            final WaterRes.DataBean.BubbleBean water = waters.get(i);
            View view = mInflater.inflate(mChildViewRes, this, false);
            TextView tvWater = view.findViewById(R.id.tv_water);
            view.setTag(water);
            tvWater.setText(String.valueOf(water.integral));
            TextView tvWaterTitle = view.findViewById(R.id.tv_water_title);
            tvWaterTitle.setText(String.valueOf(water.integralCover));
            TextView tvWaterShow = view.findViewById(R.id.tv_water_show);
            if (water.expire){
                tvWaterShow.setVisibility(VISIBLE);
            }else {
                tvWaterShow.setVisibility(GONE);
            }
    ...
    private void handViewClick(View view) {
        ...
        if (tag instanceof WaterRes.DataBean.BubbleBean) {
            WaterRes.DataBean.BubbleBean waterTag = (WaterRes.DataBean.BubbleBean) tag;
            mTotalConsumeWater += waterTag.integral;
            onViewClickListener.onViewClick(waterTag);
//            Toast.makeText(getContext(), "当前点击的是：" + waterTag.remark + "水滴的值是:"
//                    + waterTag.remark + "总的水滴数是" + mTotalConsumeWater, Toast.LENGTH_SHORT).show();
        }
    ...
    public interface onViewClickListener{
        void onViewClick(WaterRes.DataBean.BubbleBean dataBean);
    }
    ...
    private void setChildViewLocation(View view) {
        double x=maxX * getX_YRandom(mXCurrentCanShoseRandoms, mXRandoms);
        double y=maxY * getX_YRandom(mYCurrentCanShoseRandoms, mYRandoms);
        ToastUtils.e("X_"+maxX+"=="+x," Y_"+maxY+""+y);
        view.setX((float) (x));
        view.setY((float) (y));
        view.setTag(R.string.original_y, view.getY());
    }
    ...
    private double getX_YRandom(List<Float> choseRandoms, List<Float> saveRandoms) {
        ...
        int ss=mRandom.nextInt(choseRandoms.size());
        ToastUtils.i("getX_YRandom__"+choseRandoms.size(),"________"+ss);
        float random = choseRandoms.get(ss);
