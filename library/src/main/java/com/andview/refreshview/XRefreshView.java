...
//        if(mHolder.mOffsetY!=0)return;
    ...
    //判断是否是横向滑动  cwp
    private int startX, startY;//获得按下时的X Y坐标
    private int movedX, movedY;//移动时候的X Y坐标
    private int offsetX, offsetY;// 获得X Y轴的偏移量
    ...
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //添加横向判断 如果是横向则不加载或者刷新  cwp  添加if else
                movedX = (int) ev.getRawX();
                movedY = (int) ev.getRawY();
                offsetX = startX - movedX;
                offsetY = startY - movedY;
                if (Math.abs(offsetX) > Math.abs(offsetY)) {
                    //若X轴的偏移量大于Y轴的偏移量，则表示在横向滑动，否则在纵
                    //向滑动，若相同，则表示移动的时候没有移动，这里滑动的时候， 会不断的调用这个，一次滑动的距离等于多次的偏移量相加
                } else {
                    ...
                    mLastY = currentY;
                    mLastX = currentX;
                    // intercept the MotionEvent only when user is not scrolling 拦截位移事件只有当用户不滚动
                    if (!isIntercepted) {
